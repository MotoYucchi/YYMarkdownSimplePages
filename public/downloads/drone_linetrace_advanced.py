...
# プリセット適用関数
def apply_color_preset(color_name):
    global window_title
    if color_name in COLOR_PRESETS:
        preset = COLOR_PRESETS[color_name]
        cv2.setTrackbarPos("H_min", window_title, preset['h_min'])
        cv2.setTrackbarPos("H_max", window_title, preset['h_max'])
        cv2.setTrackbarPos("S_min", window_title, preset['s_min'])
        cv2.setTrackbarPos("S_max", window_title, preset['s_max'])
        cv2.setTrackbarPos("V_min", window_title, preset['v_min'])
        cv2.setTrackbarPos("V_max", window_title, preset['v_max'])
        print(f"Applied {color_name} preset")
    ...
    while True:
        ...
        # 追跡モードをOFF
            ...
            if b < 0:
                b = 0
        # 色プリセット適用のキー
        elif key == ord('5'):  # 赤色プリセット
            apply_color_preset('red')
            command_text = "Red preset"
        elif key == ord('6'):  # 青色プリセット
            apply_color_preset('blue')
            command_text = "Blue preset"
        elif key == ord('7'):  # 黄色プリセット
            apply_color_preset('yellow')
            command_text = "Yellow preset"
